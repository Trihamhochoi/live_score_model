...
class Engine:
    ...
    def get_logs_for_current_match(self):
        ...
        if len(js_rb) == 0 and len(self.full_rb_events) == 0:
            ...
            if len(js_rb) == 0:
                ...
                if len(update_ls) > 0:
                    ...
                    print(f"Target time is updated to {self.Timer.target_time}\nUpdate list: {update_ls}")
                else:
                    in_game_timer = self.Timer.create_time_counter(event_code_id=None)
                ...
                for i, event in enumerate(js_rb):
                    ...
                    if event['in_game_period'] in [0, 10, 20]:
                        # Update counter
                        rb_data = {'ingame_Timer': in_game_timer,
                                   'Event': event_counter,
                                   'LastEvent': last_event_counter}
        ...
        if admin_data['ingame_Timer']['in_game_period'] in [0, 11, 22]:
            ...
            print(f" --- Fetching data at period: {ig_['in_game_period']}\t{ig_['min']}:{ig_['sec']}")
            return admin_data
