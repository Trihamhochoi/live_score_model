...
class LiveScore_Model:
    ...
    def _decrease_both_ab(self, ability_json):
        """
        ...
        """
        try:
            lookback_ab_11s = self.ability_match[-11:]
            lookback_ab_15s = self.ability_match[(-11 - 4):]
            cond_1_1 = (lookback_ab_11s[0]['has_event'] == 'SAFE') and (lookback_ab_11s[0]['update'] is None)
            cond_1_2 = (lookback_ab_15s[0]['has_event'] == 'SAFE') and (lookback_ab_15s[0]['update'] is False)

            cond_2 = all([True if ab_sec['has_event'] not in ['AT', 'DAN', 'DAT', 'SONT', 'SWW', 'DFK', 'FK'] else False for ab_sec in lookback_ab_11s])
            ...
            if all([cond_1_1, cond_2, cond_3, cond_4]) or all([cond_1_2, cond_2, cond_3, cond_4]):
                exp_max = math.log(4)  # maximum decrease 4%
    ...
    def _transform_atk_rd(self, atk_round):
        ...
        :return: a attacking round (dataframe)
        """
        ...
        features = ['period', 'start_sec', '_mins', '_secs', 'duration', 'field', 'counter_attack',
                    'SAFE', 'TI', 'CR', 'AT', 'DAT', 'DAN', 'FK', 'DFK', 'PEN', 'Score', 'SONT', 'SWW']
        for feat in features:
            if feat not in sample.columns:
                sample[feat] = 0
        sample['goal_ab'] = sample['Score'] + sample['SONT'] * 0.5 + sample['SWW'] * 0.8
        features.append('goal_ab')
        sample = sample[features].drop(columns=['Score', 'SONT', 'SWW'])
        return sample
    ...
    def _measure_team_performance(self,
        ...
        # print(f"Rows of Home attacking round: {full_match_df[(full_match_df['field']=='home')].shape[0]}.\n\t Rows of Home Danger round: {home_atk.shape[0]}")
        # print(f"Rows of Away attacking round: {full_match_df[(full_match_df['field']=='away')].shape[0]}.\n\t Rows of Away Danger round: {away_atk.shape[0]}")
        # print(f'Attacking Performance of HOME team: {round(home_perf_score*100,2)}% \t Attacking Performance of AWAY team: {round(away_perf_score*100,2)}% \n')
    ...
    def _get_next_perf_per(self, home: True):
        ...
        if home:
            ...
            elif (self.ability_match[-1]['home_perf_coef'] != 0 and self.ability_match[-1]['away_perf_coef'] == 0) or (
                    self.ability_match[-1]['home_perf_coef'] == 0 and self.ability_match[-1]['away_perf_coef'] != 0):
    ...
    def _determine_when_attacking(self, event_dictionary):
        ...
        if (event_dictionary['type'] == 'SAFE') or (event_dictionary['sec'] <= 2) or (event_dictionary['type'] == 'Score'):
            ...
            # In Case home percent increase of Home/Away percent is greater than 0=> decrease gradually
                ...
                decrease_array = [(math.e ** exp) for exp in np.linspace(start=exp_max, stop=0, num=5)]  # decrease in 5 sec
        ...
        # Check the next event, if event is same field, => add that event to the current round:
            ...
            if (event_dictionary['type'] in ['DAT', 'DAN', 'DFK']) and (self.attacking_round[-2]['type'] in ['DAT', 'DAN', 'DFK']):
                ...
                if len(self.full_atk_rd_match) > 0:
                    ...
                    if perf_dict['home'] > 0 or perf_dict['away'] > 0:
                        print(f"""
                                    Information of point triggered the performance:
                                    \tPeriod {self.ability_json['period']}  mins {self.ability_json['second'] // 60}  sec {self.ability_json['second'] % 60}
                                    \tField {self.ability_json['field']}  Event {self.ability_json['has_event']}
                                    \tHome Performance: {perf_dict['home']} Away Performance {perf_dict['away']}
                                    """)
                else:
    ...
    def predict_expected_ability(self, input_api_json):
        ...
        order_key_h = ['SONT1', 'SWW1', 'Score1', 'SAFE1', 'TI1', 'ATT1', 'DATT1', 'DAN1', 'FK1', 'DFK1', 'CR1']
        order_key_a = ['SONT2', 'SWW2', 'Score2', 'SAFE2', 'TI2', 'ATT2', 'DATT2', 'DAN2', 'FK2', 'DFK2', 'CR2']
        ...
        try:
            ...
            # --------------------------------------------------------------------------------------------- #
            # IF AFTER 10 MINUTES, PERFORMANCE OF BOTH TEAM ARE 0 => DECREASE BOTH TEAM'S PERFORMANCE
            if self.ability_json['period'] == 2 and self.ability_json['second'] <= 2:
                self.timer_10m = 0
                self.decrease_percent = 3
            elif self.ability_json['has_event'] in ['SONT', 'Score', 'SWW']:
                self.timer_10m = 0
                decrease_percent = 3
                self.ability_json['home_perf_coef'] = 0
                self.ability_json['away_perf_coef'] = 0
                ...
                self.ability_json['home_perf_coef'] = 0
                self.ability_json['away_perf_coef'] = 0
                if (10 * 60) <= self.timer_10m < 20 * 60:
                    print(f'Decrease level 1 with timer: {self.timer_10m}')
                    self.ability_json['home_perf_coef'] = -0.01
                    self.ability_json['away_perf_coef'] = -0.01
                    print(
                        f"\t Period: {self.ability_json['period']} -- second: {self.ability_json['second']} -- Home Performance: {self.ability_json['home_perf_coef']} Away Performance: {self.ability_json['away_perf_coef']}")
                elif self.timer_10m >= (20 * 60):
            ...
            # DECREASE ABILITIES OF HOME AND AWAY TEAM WHEN THERE IS NO ATTACK DURING THE MATCH.
                ...
                self.ability_json = self._decrease_both_ab(ability_json=self.ability_json)
...
if __name__ == '__main__':
    ...
    f = open('json/Germany_3rd_liga/74413938_FT.json')
