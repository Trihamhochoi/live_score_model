...
class LiveScore_Model:
    ...
    def _predict_percent(self, atk_round: list):
        ...
        # a team is slightly stronger than the other team
            ...
            if diff > 0:
                mapping = {'home': {'Low': 7, 'Medium': 12, 'High': 15},
                           'away': {'Low': 5, 'Medium': 10, 'High': 10}}
            else:
                mapping = {'home': {'Low': 5, 'Medium': 10, 'High': 10},
                           'away': {'Low': 7, 'Medium': 12, 'High': 15}}
        ...
        if home:
            if self.ability_full_match_list[-1]['home_percent_increase'] == 0:
                ...
                try:
                    ...
                    percent = (math.e ** has_update['percent_exponent_array'][int(index)]) / 100
                    # If percent is 1% => go back 0
                    if percent <= 0.01:
                        percent = 0
    ...
    def _determine_when_attacking(self, event_dictionary):
        ...
        if (event_dictionary['type'] == 'SAFE') or (event_dictionary['sec'] < 2) or (event_dictionary['type'] == 'Score'):
            ...
            # Determine the HOME/ AWAY percent
            self.ability_json['field'] = event_dictionary['field']
            if len(self.ability_full_match_list) == 0 or (event_dictionary['period'] == 2 and event_dictionary['sec'] < 2):
            ...
            elif self.ability_full_match_list[-1]['home_percent_increase'] == 0 and self.ability_full_match_list[-1]['away_percent_increase'] == 0:
                self.ability_json['home_percent_increase'] = 0
                self.ability_json['away_percent_increase'] = 0
                self.ability_json['percent_exponent_array'] = None

            # In Case home percent increase of Home/Away percent is not 0=> decrease gradually
            elif self.ability_full_match_list[-1]['home_percent_increase'] != 0:
                # Get exponent for evnet Safe and decreased array
                max_decrease = self.ability_full_match_list[-1]['home_percent_increase']
                exp_max = math.log((max_decrease - 0.005) * 100)  # the exponent for Safe event
                decrease_array = np.linspace(start=exp_max, stop=0, num=10)  # decrease in 10 sec

                # Update ability
                self.ability_json['update'] = False
                self.ability_json['away_percent_increase'] = 0
                self.ability_json['home_percent_increase'] = (math.e ** decrease_array[0]) * 0.01
                self.ability_json['percent_exponent_array'] = decrease_array

            elif self.ability_full_match_list[-1]['away_percent_increase'] != 0:
                # Get exponent for evnet Safe and decreased array
                max_decrease = self.ability_full_match_list[-1]['away_percent_increase']
                exp_max = math.log((max_decrease - 0.005) * 100)  # the exponent for Safe event
                decrease_array = np.linspace(start=exp_max, stop=0, num=10)  # decrease in 10 sec
                ...
                self.ability_json['update'] = False
                self.ability_json['home_percent_increase'] = 0
                self.ability_json['away_percent_increase'] = (math.e ** decrease_array[0]) * 0.01
                self.ability_json['percent_exponent_array'] = decrease_array
            ...
            elif event_dictionary['type'] in ['AT'] and self.attacking_round[-2]['type'] == 'SAFE':
                self.ability_json['field'] = event_dictionary['field']
                self.ability_json['home_percent_increase'] = 0
                self.ability_json['away_percent_increase'] = 0
                self.ability_json['percent_exponent_array'] = None
    ...
    def predict_expected_ability(self, input_api_json):
        ...
        self.ability_json = {'period': timer['LivePeriod'],
                             ...
                             'has_event': None,
                             'update': None,
                             'home_percent_increase': None, 'away_percent_increase': None,
