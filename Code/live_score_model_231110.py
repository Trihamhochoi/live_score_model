...
class LiveScore_Model:
    ...
    def _predict_percent(self, atk_round: list):
        ...
        # a team is rapidly stronger than the other team
            ...
            if diff > 0:
                mapping = {'home': {'Low': 6, 'Medium': 10, 'High': 10},
                           'away': {'Low': 4, 'Medium': 7, 'High': 7}}
            else:
                mapping = {'home': {'Low': 4, 'Medium': 7, 'High': 7},
                           'away': {'Low': 6, 'Medium': 10, 'High': 10}}
    ...
    def _decrease_both_ab(self, ability_json):
        ...
        """
        try:
            lookback_ab_11s = self.ability_full_match_list[-12:]
            cond_1 = (self.attacking_round[-1]['type'] == 'SAFE')
    ...
    def _determine_when_attacking(self, event_dictionary):
        ...
        if (event_dictionary['type'] == 'SAFE') or (event_dictionary['sec'] <= 2) or (event_dictionary['type'] == 'Score'):
            ...
            if (
                    len(self.ability_full_match_list) == 0 or
