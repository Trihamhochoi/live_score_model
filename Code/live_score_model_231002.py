...
class LiveScore_Model:
    ...
    def _predict_percent(self, atk_round: list):
        ...
        # get percent:
        mapping = {'Low': 0.05, 'Medium': 0.07, 'High': 0.1}
        ...
        if (event_dictionary['type'] == 'SAFE') or (event_dictionary['sec'] < 2):
            ...
            if (event_dictionary['type'] in ['DAT', 'DAN', 'DFK']) and (self.attacking_round[-2]['type'] in ['DAT', 'DAN', 'DFK']):
                ...
                if event_dictionary['field'] == 'home':
                    self.ability_json['home_percent_increase'] = get_percent
                    self.ability_json['away_percent_increase'] = 0
                else:
                    self.ability_json['home_percent_increase'] = 0
                    self.ability_json['away_percent_increase'] = get_percent
            else:
                self.ability_json['home_percent_increase'] = self.ability_full_match_list[-1]['home_percent_increase']
                self.ability_json['away_percent_increase'] = self.ability_full_match_list[-1]['away_percent_increase']

            # In case as counterstrike, still add to that current round but this is rival's attacking round.
        elif event_dictionary['field'] != self.attacking_round[-1]['field'] and event_dictionary['type'] not in ['TI',
                                                                                                                 'CR']:
...
if __name__ == '__main__':
    ...
    # Call model
    pickle_file_path = 'Code/model/rf_model.sav'
    ...
    full_ability = None
    for idx, dict_sec in enumerate(api_json[0:50]):
        if idx ==0:
    ...
    #print(live_model.attacking_round)